    ...
    config.vm.define "consul" do |consul|
        ...
        (1..regions.length() ).each do |idx|
            ...
            consul.trigger.after :up do |trigger|
                ...
                trigger.run = {inline: "bash -c 'export version_number=1; cat nginx.conf.tmpl | envsubst > #{local_nginx_file};
        ...
        regions.each do |region|
            consul.trigger.after :up do |trigger|
               ...
               trigger.run = {inline:  "bash -c '(nomad agent -config nomad-#{region}.hcl &> logs/#{region}.log) &'" }
            end
        end
